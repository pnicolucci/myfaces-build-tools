...
public class BehaviorParsingStrategy extends ClassMetaPropertyParsingStrategy
{
    ...
    public void parseClass(JavaClass clazz, Model model)
    {
        ...
        JavaAnnotation anno;
        // behaviors
        tag = clazz.getTagsByName(DOC_BEHAVIOR, false).get(0);
        if (tag != null)
        {
            Map props = tag.getNamedParameterMap();
            processBehavior(props, (JavaAnnotatedElement)tag.getContext(), clazz, model);
        }
        anno = QdoxHelper.getAnnotation(clazz, DOC_BEHAVIOR);
        if (anno != null)
        {
            Map props = anno.getNamedParameterMap();
            processBehavior(props, (JavaAnnotatedElement)anno, clazz, model);
        }
    }

    private void processBehavior(Map props, JavaAnnotatedElement ctx,
